...
if str2double(vernos{strcmpi({vv.Name},'MATLAB')}) < 7.7
    ...
    state.LinprogOptions = optimset(...'Simplex','off',
        ...
        'Display','off') ; %'LargeScale','off',...   % commented on Feb 19 2018 by Pan Zhao
end
...
for i = 1:size(state.Population,1)
    if strcmpi(options.LinearConstr.type,'linearconstraints')
        if (~isempty([Aineq,bineq]) && any(Aineq*state.Population(i,:)' - bineq > options.TolCon)) ...
            ...
            % modified by Pan Zhao to add some randomness to the
            % repositioned particles through the cost function
            nvars = size(state.Population,2); % dimension of a partile
            [newpoint,~,exitflag] = ...
                linprog(rand(1,nvars)*2-1,Aineq,bineq,Aeq,beq,LB,UB,...
                state.Population(i,:),...
