function [xOpt,fval,exitflag,output,population,scores,iterations] = ...
    pso_ssopt(fitnessfcn,nvars,Aineq,bineq,Aeq,beq,LB,UB,nonlcon,options)
...
%
% % [x,fval,exitflag,output,population,scores,iterations] = pso(...)
% A structure contain the local best and global best in all iterations
...
% -------------------------------------------------------------------------
ss_dist_min = abs(bineq(1));                    % minimum disturbance between switching surfaces 
if strncmpi(options.PopulationType,'double',2)
    state = psocreationuniform_ssopt(options,nvars,ss_dist_min) ;
...
% added by Pan Zhao on Feb 25 2018
iterations.fGlobalBest = ones(1,itr+1)*nan;
iterations.fLocalBest = ones(n,itr+1)*nan;
iterations.xGlobalBest = ones(1,nvars,itr+1)*nan;
iterations.xLocalBest = ones(n,nvars,itr+1)*nan;
tic
for k = 1:itr
...
%             matlabpool('addattachedfiles',{[pwd '\testfcns\nonlinearconstrdemo.m']}) ;
    ...
    % added by Pan Zhao
    iterations.fLocalBest(:,k) = state.fLocalBests;
    iterations.xLocalBest(:,:,k) = state.xLocalBests;
...
%     tempchk = alpha <= 1.6 ;
    if k == 1 || minfitness < state.fGlobalBest(k-1)
        ...
        imprvchk = k > options.StallGenLimit && ...
            (state.fGlobalBest(k - options.StallGenLimit) - ...
                state.fGlobalBest(k)) / abs(state.fGlobalBest(k - options.StallGenLimit)) < ...
                options.TolFun ;
        if imprvchk
            exitflag = 1 ;
            flag = 'done' ;
    ...
    % added by Pan Zhao
    iterations.fGlobalBest(:,k) = state.fGlobalBest(k);
    iterations.xGlobalBest(:,:,k) = state.xGlobalBest;
...
% added by Pan Zhao on Feb 25 2018: 
iterations.fGlobalBest = iterations.fGlobalBest(:,1:k);
iterations.fLocalBest = iterations.fLocalBest(:,1:k);
iterations.xGlobalBest = iterations.xGlobalBest(:,:,1:k);
iterations.xLocalBest = iterations.xLocalBest(:,:,1:k);
